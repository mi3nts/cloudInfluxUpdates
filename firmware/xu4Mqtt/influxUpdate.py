...
# Steps 
#   1) Read all nodeIDs from a cloud based .csv 
#   2) For each nodeID sync data to influx 
#   3) Start by just one    
#   4) Corrections should be considered too
#   5) Stratergy to get the latest updates
...
#  With on board sensors:
    # Should go through all node IDs 
    # Some means to check if connectivity is there 
    # Some way to keep the latest time stamp synced
...
def main():    
    ...
    while True:
        try:
            if is_connected():
                for index, row in nodeInfo.iterrows():
                    nodeID = row['mac_address']
                    nodeName = row['name']
